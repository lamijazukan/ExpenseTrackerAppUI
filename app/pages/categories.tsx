...
import { mockCategories, type Category, type Subcategory } from "@/app/lib/mock-data"
  ...
  return (
    ...
    >
      ...
      <DialogContent className="border-border bg-card text-card-foreground sm:max-w-sm">
        <DialogHeader>
          <DialogTitle className="text-card-foreground">Edit Subcategory</DialogTitle>
          <DialogDescription className="text-muted-foreground">
...
function AddSubcategoryForm({
  ...
  return (
    ...
    >
      <p className="text-xs font-medium text-muted-foreground">New Subcategory</p>
...
function CategoryCard({
  ...
  onEditCategory: (catId: string, data: { name: string; icon: string; budget: number }) => void
  onDeleteCategory: (catId: string) => void
  onEditSubcategory: (catId: string, subId: string, data: { name: string; budget: number }) => void
          ...
          <div className="border-t border-border">
            {category.subcategories.map((sub) => (
              <SubcategoryRow
                ...
                subcategory={sub}
                onEdit={(data) =>
                  onEditSubcategory(category.id, sub.id, data)
                }
...
export default function CategoriesPage() {
  ...
  function handleAddCategory(data: { name: string; icon: string; budget: number }) {
        ...
        c.id === catId
          ? {
              ...
              subcategories: c.subcategories.map((s) =>
                s.id === subId ? { ...s, name: data.name, budget: data.budget } : s
              ),
            }
          : c
      )
